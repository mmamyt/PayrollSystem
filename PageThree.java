...
public class PageThree extends javax.swing.JFrame {
    ...
    private JLabel labelID, labelFN, labelLN, labelRole, labelSDate, labelSalary, labelDepartment, labelSSN, labelEmail, success;
    private JTextField ID, firstName, lastName, role, startDate, salary, department, ssn, email;
    ...
    public PageThree() {
        ...
        labelSSN.setBounds(20, 130, 100, 25);
        panel.add(labelSSN);
        ssn = new JTextField();
        ssn.setBounds(100, 130, 150, 25);
        panel.add(ssn);
        ...
        buttonAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (!ID.getText().isEmpty() && !firstName.getText().isEmpty() && !lastName.getText().isEmpty() && !role.getText().isEmpty() && !startDate.getText().isEmpty() &&
                        !salary.getText().isEmpty() && !department.getText().isEmpty() && !ssn.getText().isEmpty() && !email.getText().isEmpty()) {
                    try {
                        String query = "INSERT INTO Employees (ID, First_Name, Last_Name, Role, Salary, StartDate, EndDate, Department, SSN, email) VALUES ('" +
                                ID.getText() + "','" + firstName.getText() + "','" + lastName.getText() + "','" + role.getText() + "'," + salary.getText()
                                + ", '" + startDate.getText() + "', 'null', '" + department.getText() + "','" + ssn.getText() + "','" + email.getText() + "')";
